<?php  // $Id: viewbe.php,v 1.0 2012/06/26 19:40:00 Ana María Lozano de la Fuente Exp $
/*********************************************************************************
...
*********************************************************************************/
	...
	// Log table
    add_to_log($course->id, "genetic", "view departaments (be)", "viewbe.php?id=$cm->id", "$genetic->id");
    ...
    // Print Title
	print_heading(get_string('showbe', 'genetic'), 'center',2);	
	...
	while ($row = mysql_fetch_array($result)) {
		$idbe = $row['id'];
		$name = $row['name'];
		$action = "<A HREF=\"editbe_form.php?id={$cm->id}&idbe=$idbe&action=edit\"><IMG SRC=\"images/edit.gif\" tittle=\"".$striconedit."\" ALT=\"".$striconedit."\"></A>&nbsp;
				   <A HREF=\"editbe_form.php?id={$cm->id}&idbe=$idbe&action=delete\"><IMG SRC=\"images/delete.gif\" tittle=\"".$stricondelete."\" ALT=\"".$stricondelete."\"></A>";
	...
	$origen="add_be";
	// Check if the user has permission in this activity to add departments
	if (has_capability('mod/genetic:manageentries', $context)) {
		// Form to add a department
		echo "<TABLE WIDTH=\"100%\">";
		echo "<FORM NAME=\"addbeform\" METHOD=\"post\" ACTION=\"editbe_form.php?id=$id&origen=$origen\" ENCTYPE=\"multipart/form-data\">";
		echo "<TR><TD ALIGN=\"center\"><BR /><BR />";
		echo "<INPUT TYPE=\"submit\" VALUE=\"".$str = get_string("buttonaddbe", "genetic")."\" NAME=\"buttonadd\" />&nbsp;&nbsp;";
