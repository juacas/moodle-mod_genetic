...
$dom = optional_param('dom', '', PARAM_TEXT);
...
//	Get the necessary strings
	...
	$strsubdomain = get_string("subdomain", "genetic");
			...
			for($z=0;$z<$numpag;$z++){
				...
				if($n!=0){
					...
					for($i=0; $i<4; $i++){
						...
						$cardrowidheader = stripslashes($cardrow['id']);
						...
						for ($k=0; $k<$ncards; $k++) {
							...
							$cardrowterm = stripslashes($cardrow['term']);
									...
									if ($cardrowterm != '') {
										...
										$pdf->Image('images2\\'.$cardrowisolang.'.jpg',10+50*$k,20+5*$i+45*($i+1),5,5);
