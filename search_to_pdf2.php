...
$term = utf8_decode(optional_param('term', '', PARAM_TEXT));
...
//NUEVAS CADENAS
	...
	$strwm=utf8_decode(get_string("wm", "genetic"));	
			...
			$resultterm = mysql_query($query,$link);
			
			//NUMBER OF MATCHES==number of match terms 
			$n = mysql_num_rows($resultterm);
			
			
			// COUNT THE RESULTS
			$query22 = genetic_search_term($genetic->id, $term);
			$resultterm22 = mysql_query($query22,$link);
			$numpag2=0;
			for($w=0;$w<$n;$w++){
			
				$cardrowheader2 = mysql_fetch_array($resultterm22);
				$cardrowidheader2 = stripslashes($cardrowheader2['idheader']);
				//count how many terms for each header
				$query22 = genetic_show_cards_dic($cardrowidheader2,$genetic->id);
				$resultc2 = mysql_query($query22, $link);
							
				//NUMBER OF CARDS MATCHES
				$aux = mysql_num_rows($resultc2);
				$numpag2=$numpag2+$aux;
			}
			...
			$cardrowheader = mysql_fetch_array($resultterm);
			$cardrowidheader = stripslashes($cardrowheader['idheader']);
			//count how many terms for each header
			$query2 = genetic_show_cards_dic($cardrowidheader,$genetic->id);
			$resultc = mysql_query($query2, $link);
							
			//NUMBER OF CARDS MATCHES
			$numpag = mysql_num_rows($resultc);
									...
									if ($cardrowterm != '') {
										...
										//echo"<TR><TD><IMG SRC=\"images/Picture.gif\"><B>".$strimagenes."</B></TD><TD></TD><TD><B>".$strsrc."</TD></TR>";
											...
											//---añadido---mostrar imagenes
												...
												//$rutaDestino='C:\wamp/moodledata/'.$COURSE->id.'/imagen/'.$cardrowfile_image;
												...
												$pdf->Ln();
												$pdf->Cell(12);
												$pdf->Image($rutaDestino,25,$linea+10,5,5,'JPG');
										...
										$pdf->Cell(12);
										$pdf->SetFont('Arial','B',8);
										$pdf->Cell(74,5,$strvideos);
										$pdf->SetFont('Arial','B',8);
										//$pdf->Cell(24);
										$pdf->Cell(30,5,$straudio.':');
										$pdf->SetFont('Arial','B',8);
										//$pdf->Cell(12);
										$pdf->Cell(50,5,$strsources.':');
										...
										//echo"<TR><TD><B>".$strvideos."</B></TD><TD></TD><TD><B>".$straudio.":</TD><TD><B>".$strsrc."</TD></TR>";
											while($cardrow4 = mysql_fetch_array($resultc4))
											{
												...
												if($cardrowtitle_video!=''){
													...
													$pdf->SetFont('Arial','',8);
													//$pdf->Cell(10);
													$pdf->Cell(74,5,utf8_decode($cardrowtitle_video));
													...
													$pdf->SetFont('Arial','',8);
													$pdf->Cell(10);
													$pdf->Cell(74,5,utf8_decode($cardrowfile_video));
													...
													//$pdf->Cell(12);
													$pdf->Cell(50,5,utf8_decode($cardrowsrc_video));
											}
										}//fin de videos
