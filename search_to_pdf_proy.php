...
$idty = utf8_decode(optional_param('proyect',0,PARAM_INT));
...
//NUEVAS CADENAS
	...
	$strwm=utf8_decode(get_string("wm", "genetic"));	
			...
			//Search the name of the proyect
			
			$query2 = genetic_search_ty($idty);
			$resultty = mysql_query($query2,$link);
			$headerrowty = mysql_fetch_array($resultty);
			$headerrow_ty = stripslashes($headerrowty['name']);
									...
									if ($cardrowterm != '') {
										...
										//echo"<TR><TD><IMG SRC=\"images/Picture.gif\"><B>".$strimagenes."</B></TD><TD></TD><TD><B>".$strsrc."</TD></TR>";
											...
											//---añadido---mostrar imagenes
												...
												$pdf->Ln();
												$pdf->Cell(12);
												$pdf->Image($rutaDestino,25,$linea+10,5,5);
