...
$dom = utf8_decode(optional_param('dom', '', PARAM_TEXT));
...
//	Get the necessary strings
	...
	$strdom = utf8_decode(get_string("dom", "genetic"));
	$strsubdom = utf8_decode(get_string("subdom", "genetic"));
	$strsubdomain = utf8_decode(get_string("subdomain", "genetic"));
	$strauthor = utf8_decode(get_string("author", "genetic"));
...
//NUEVAS CADENAS
	...
	while($flag==1){		
			...
			$resultheader = mysql_query($query,$link);
			
			
			//Search the name of the subdomain
			$query2 = genetic_search_domain_name($dom);
			$result2 = mysql_query($query2,$link);
			$subdomainrow = mysql_fetch_array($result2);					
			// Remove '\' from the entries and allow them in variables
			$subdomain_name = stripslashes($subdomainrow['name']);
			
			//NUMBER OF MATCHES==number of pages I need
			$n = mysql_num_rows($resultheader);
			...
			// Make the card search; for every card
							
			$querycard = genetic_show_cards_dic($headerrowni,$genetic->id);
			$resultc = mysql_query($querycard, $link);
			...
			for($z=0;$z<$numpag;$z++){
				...
				if($z==0){
					...
					//$pdf->Cell(35);
					$pdf->SetFont('Arial','',8);
					$pdf->Cell(40,5,$strsubdomain);
					
					$pdf->SetFont('Arial','B',8);
					$pdf->Cell(40,5,$strsubdomain.': ');
					//$pdf->Cell(35);
					$pdf->SetFont('Arial','',8);
					$pdf->Cell(40,5,utf8_decode($subdomain_name));
