<?php  //$Id: upgrade.php,v 1.0 2009/08/10 18:00:00 Irene Fernández Ramírez Exp $
...
function xmldb_genetic_upgrade($oldversion=0) {
...
/// First example, some fields were added to the module on 20070400
    if ($result && $oldversion < 20014012704) {
		$result &= execute_sql("ALTER TABLE `mdl_genetic_cards` MODIFY definition text");
        $result &= execute_sql("ALTER TABLE `mdl_genetic_cards` MODIFY context text");
        $result &= execute_sql("ALTER TABLE `mdl_genetic_remission` MODIFY remission text");
		$result &= execute_sql("ALTER TABLE `mdl_genetic_sources` MODIFY srcterm text");
        $result &= execute_sql("ALTER TABLE `mdl_genetic_sources` MODIFY srcdefinition text");
        $result &= execute_sql("ALTER TABLE `mdl_genetic_sources` MODIFY srccontext text");
        $result &= execute_sql("ALTER TABLE `mdl_genetic_sources` MODIFY srcexpression text");
        $result &= execute_sql("ALTER TABLE `mdl_genetic_sources` MODIFY srcrv text");
        $result &= execute_sql("ALTER TABLE `mdl_genetic_sources` MODIFY srcnotes text");
