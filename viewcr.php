<?php  // $Id: viewty.php,v 1.0 2012/06/26 19:40:00 Ana María Lozano de la Fuente Exp $
    ...
    // Print Title
	print_heading(get_string('showcr', 'genetic'), 'center',2);	
	...
	//Show a table with all related terms
	...
	$lang="de";
	$query = genetic_show_crossrelated($lang);
	$result = mysql_query($query,$link);
	while ($row = mysql_fetch_array($result)) {
		$idcr = $row['id'];
		$name = $row['name'];
		$action = "<A HREF=\"editcr_form.php?id={$cm->id}&idcr=$idcr&action=edit\"><IMG SRC=\"images/edit.gif\" tittle=\"".$striconedit."\" ALT=\"".$striconedit."\"></A>&nbsp;
				   <A HREF=\"editcr_form.php?id={$cm->id}&idcr=$idcr&action=delete\"><IMG SRC=\"images/delete.gif\" tittle=\"".$stricondelete."\" ALT=\"".$stricondelete."\"></A>";
	...
	if (has_capability('mod/genetic:manageentries', $context)) {
		...
		echo "<FORM NAME=\"addcrform\" METHOD=\"post\" ACTION=\"editcr_form.php?id=$id\" ENCTYPE=\"multipart/form-data\">";
		echo "<TR><TD ALIGN=\"center\"><BR /><BR />";
		echo "<INPUT TYPE=\"submit\" VALUE=\"".$str = get_string("buttonaddcr", "genetic")."\" NAME=\"buttonadd\" />&nbsp;&nbsp;";
