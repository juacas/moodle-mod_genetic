<?php  // $Id: editty.php,v 1.0 2012/06/14 10:45:00 Ana María Lozano de la Fuente Exp $
	...
	// Log table
    add_to_log($course->id, "genetic", "edit related term", "editrt.php?id=$cm->id&idrt=$idrt&action=$action", "$genetic->id");
	...
	if ($action == "delete") {
		...
		$query = genetic_delete_rt($idrt);
		$result = mysql_query($query,$link);
		$nok = mysql_affected_rows($link);
		
		// Delete ok or not?
		if($nok == 0) {
			$redirectmsg = get_string("deletertnok", "genetic");
		...
		$redirectmsg = get_string("deletertok", "genetic")."<BR />";
		redirect($url="viewrt.php?id={$cm->id}", $redirectmsg, $delay=-1);
		// Close the db    
		mysql_close($link);
		// Finish the page
		...
		if ($name == "")  {
			...
			if ($action == "") {
				...
				//Comprobar si el sinónimo ya existe en ese idioma
							...
							$query = genetic_related_exists($name,$language);			
							$result=mysql_query($query,$link);
							$nok = mysql_affected_rows($link);				
							
							if($nok==0){	
			
								$query = genetic_insert_rt($name,$language);
								$result = mysql_query($query,$link);
								$nok = mysql_affected_rows($link);

								// Insert ok or not?
								if($nok == 0) {
								$redirectmsg = get_string("insertrtnok", "genetic");
								redirect($url="viewrt.php?id={$cm->id}", $redirectmsg, $delay=-1);					
									// Close the db    
								mysql_close($link);
								// Finish the page
								...
								$redirectmsg = get_string("insertrtok", "genetic");
							...
							$redirectmsg = get_string("insertrtused", "genetic");
			...
			// Edit
				...
				$query = genetic_update_rt($idrt, $name);
				$result = mysql_query($query,$link);
				$nok = mysql_affected_rows($link);
				...
				$redirectmsg = get_string("updatertok", "genetic");
				redirect($url="viewrt.php?id={$cm->id}", $redirectmsg, $delay=-1);
				// Close the db 
				mysql_close($link);
			}
